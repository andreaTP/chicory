...
public class AotGenMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException {
        ...
        var machineFactoryName = baseName + "MachineFactory";
        var moduleName = baseName + "Module";
        var wasmName = baseName + ".wasm";

        generateMachineFactory(dest, packageName, machineFactoryName, machineName);

        generateModule(dest, packageName, moduleName, wasmName);
    ...
    private static void generateMachineFactory(
            SourceRoot dest, String packageName, String machineFactoryName, String machineName) {
        ...
        dest.add(packageName, machineFactoryName + ".java", cu);
    }

    private static void generateModule(
            SourceRoot dest, String packageName, String moduleName, String wasmName) {
        var cu = new CompilationUnit(packageName);
        ...
        var clazz = cu.addClass(moduleName, Keyword.PUBLIC, Keyword.FINAL);

        var constr = clazz.addConstructor(Keyword.PRIVATE);
        constr.createBody();

        var method = clazz.addMethod("load", Keyword.PUBLIC, Keyword.STATIC);
        method.setType(WasmModule.class);
        var methodBody = method.createBody();
