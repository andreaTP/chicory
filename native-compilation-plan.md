...
**Current: 21217 tests, 0 failures, 0 errors, 14 skipped.**
77 wast files included (incl. elem.wast). All non-simd wasts pass.
...
### Next: NativeTable redesign — funcPtr-in-table (anyfunc entries)

#### Problem

Current NativeTable stores module-relative funcIds (i32). Cross-module shared tables
break because funcId=1 means different functions in different modules. The interpreter
handles this via `Instance[]` array parallel to `refs[]`, but native code has no
equivalent — it uses the calling module's funcTable, which has wrong entries for
foreign functions.

#### Solution: store native function pointers directly in the table

Instead of `[funcId:i32]` per slot, store `[canonicalTypeIdx:i32][pad:i32][funcPtr:i64]`
(16 bytes per slot). This is the wasmtime "anyfunc" approach.

```
New table entry layout (16 bytes per slot):
  ┌────────────────────────────────────────┐
  │  [0..4)    i32  canonicalTypeIdx       │
  │  [4..8)    i32  (padding/reserved)     │
  │  [8..16)   i64  funcPtr (native addr)  │
  └────────────────────────────────────────┘
  NULL entry: funcPtr == 0

Table buffer layout:
  ┌──────────────────────────────────────────────────────┐
  │  [0..4)    i32  size          current element count  │
  │  [4..8)    i32  maxSize       max capacity           │
  │  [8..N)    entries[...]       16 bytes each           │
  └──────────────────────────────────────────────────────┘
```

#### CALL_INDIRECT with funcPtr-in-table

```
native code:
  tablePtr = load TABLE_PTRS[tableIdx]
  tableSize = load tablePtr[0]                    // i32 size
  brif elemIdx >= tableSize → trap OOB
  entryBase = tablePtr + 8 + elemIdx * 16         // 16 bytes per entry
  funcPtr = load entryBase[8]                      // i64 function pointer
  brif funcPtr == 0 → trap null
  canonicalType = load entryBase[0]                // i32 type index
  brif canonicalType != expectedCanonicalType → trap type mismatch
  call_indirect funcPtr(memBase, ctxPtr, args...)  // direct call
```

One fewer load than before (no funcTable indirection). Type check and funcPtr
are in the same cache line (16 bytes per entry).

#### Why this fixes cross-module sharing

When module2 writes `$const-i32-c` to slot 7 of the shared table:
- Old: writes funcId=0 (module2-relative) → module1 reads funcId=0, calls wrong function
- New: writes funcPtr=<module2's native code address for $const-i32-c> → module1 reads
  the same absolute address, calls the correct function

Pointers are absolute — no module-relative ambiguity.

#### Changes required

1. **NativeTable**: 16 bytes/slot instead of 4. Store `[typeIdx:i32][pad:i32][funcPtr:i64]`.
   `setRef()` must accept both typeIdx and funcPtr. `ref()` returns funcId (for Java compat)
   but native code reads funcPtr directly.
2. **NativeTable.setRef()**: Called by Instance.initialize() during elem segment init.
   Must resolve funcId → funcPtr using the defining module's funcTable. This requires
   access to the NativeMachine that owns the function.
3. **NativeEmitters.emitCallIndirect()**: Load funcPtr from table entry directly (skip
   funcTable lookup). Load typeIdx from same entry for type check.
4. **funcTypesArray**: No longer needed for call_indirect (type is in the table entry).
   Keep for other uses or remove.
5. **TABLE.GET/SET**: Load/store full 16-byte entries or just the funcPtr portion.
6. **TABLE.FILL/COPY**: Wider entries (16 bytes instead of 4).
7. **REF_NULL**: funcPtr=0 sentinel (not -1).
8. **REF_IS_NULL**: Compare funcPtr against 0.
9. **REF_FUNC**: Must resolve to funcPtr at emit time or via trampoline.
10. **NativeTable constructor**: Initialize all entries with funcPtr=0 (null).
