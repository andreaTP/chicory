...
class InterpreterMachine implements Machine {
    ...
    public Value[] call(int funcId, Value[] args) throws ChicoryException {
        var unboxedArgs = new long[args.length];
        for (int i = 0; i < args.length; i++) {
            unboxedArgs[i] = args[i].asLong();
        }
        return call(stack, instance, callStack, funcId, unboxedArgs, null, true);
    }
    ...
    public static Value[] call(
        ...
        var t = instance.type(instance.functionType(funcId));
        for (var i = totalResults - 1; i >= 0; i--) {
            results[i] = new Value(t.returns().get(i), stack.pop());
        }
        return results;
    }
    ...
    private static void F32_EQ(MStack stack) {
        var b = Value.asFloat(stack.pop());
        var a = Value.asFloat(stack.pop());
        stack.push(OpcodeImpl.F32_EQ(a, b));
    }

    private static void F64_EQ(MStack stack) {
        var b = Value.asDouble(stack.pop());
        var a = Value.asDouble(stack.pop());
    ...
    private static void F64_CONVERT_I64_U(MStack stack) {
        var tos = stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F64_CONVERT_I64_U(tos)));
    }

    private static void F64_CONVERT_I32_U(MStack stack) {
        var tos = (int) stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F64_CONVERT_I32_U(tos)));
    }

    private static void F64_CONVERT_I32_S(MStack stack) {
        var tos = (int) stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F64_CONVERT_I32_S(tos)));
    }
    ...
    private static void F64_NEAREST(MStack stack) {
        var val = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_NEAREST(val)));
    }

    private static void F32_NEAREST(MStack stack) {
        var val = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_NEAREST(val)));
    }

    private static void F64_TRUNC(MStack stack) {
        var val = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_TRUNC(val)));
    }

    private static void F64_CEIL(MStack stack) {
        var val = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_CEIL(val)));
    }

    private static void F32_CEIL(MStack stack) {
        var val = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_CEIL(val)));
    }

    private static void F64_FLOOR(MStack stack) {
        var val = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_FLOOR(val)));
    }

    private static void F32_FLOOR(MStack stack) {
        var val = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_FLOOR(val)));
    }

    private static void F64_SQRT(MStack stack) {
        var val = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_SQRT(val)));
    }

    private static void F32_SQRT(MStack stack) {
        var val = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_SQRT(val)));
    }

    private static void F64_MAX(MStack stack) {
        var a = Value.asDouble(stack.pop());
        var b = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_MAX(a, b)));
    }

    private static void F32_MAX(MStack stack) {
        var a = Value.asFloat(stack.pop());
        var b = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_MAX(a, b)));
    }

    private static void F64_MIN(MStack stack) {
        var a = Value.asDouble(stack.pop());
        var b = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_MIN(a, b)));
    }

    private static void F32_MIN(MStack stack) {
        var a = Value.asFloat(stack.pop());
        var b = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_MIN(a, b)));
    }

    private static void F64_DIV(MStack stack) {
        var a = Value.asDouble(stack.pop());
        var b = Value.asDouble(stack.pop());
        stack.push(Value.toLong(b / a));
    }
    ...
    private static void F64_PROMOTE_F32(MStack stack) {
        var tos = stack.pop();
        // TODO: this can be no-op probably, verify!
        stack.push(Value.toLong(Value.asFloat(tos)));
    }

    private static void F64_REINTERPRET_I64(MStack stack) {
        long tos = stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F64_REINTERPRET_I64(tos)));
    }

    private static void I32_WRAP_I64(MStack stack) {
        var tos = stack.pop();
        // no-op?
        // stack.push(tos.asInt());
        stack.push(tos);
    }
    ...
    private static void F32_DEMOTE_F64(MStack stack) {
        ...
        // more no-op?
        // TODO: verify!
        stack.push(Value.toLong((float) val));
    }

    private static void F32_CONVERT_I32_S(MStack stack) {
        var tos = (int) stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F32_CONVERT_I32_S(tos)));
    }
    ...
    private static void F32_COPYSIGN(MStack stack) {
        var b = Value.asFloat(stack.pop());
        var a = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_COPYSIGN(a, b)));
    }

    private static void F32_ABS(MStack stack) {
        var val = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_ABS(val)));
    }

    private static void F64_ABS(MStack stack) {
        var val = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_ABS(val)));
    }
    ...
    private static void F32_CONVERT_I32_U(MStack stack) {
        var tos = (int) stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F32_CONVERT_I32_U(tos)));
    }

    private static void F32_CONVERT_I64_S(MStack stack) {
        var tos = stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F32_CONVERT_I64_S(tos)));
    }
    ...
    private static void F64_CONVERT_I64_S(MStack stack) {
        var tos = stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F64_CONVERT_I64_S(tos)));
    }
    ...
    private static void I64_TRUNC_F32_S(MStack stack) {
        var tos = Value.asFloat(stack.pop());
    ...
    private static void I32_TRUNC_F64_S(MStack stack) {
        var tos = Value.asDouble(stack.pop());
    ...
    private static void F32_CONVERT_I64_U(MStack stack) {
        var tos = stack.pop();
        stack.push(Value.toLong(OpcodeImpl.F32_CONVERT_I64_U(tos)));
    }
    ...
    private static void F64_COPYSIGN(MStack stack) {
        var b = Value.asDouble(stack.pop());
        var a = Value.asDouble(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F64_COPYSIGN(a, b)));
    }

    private static void F32_TRUNC(MStack stack) {
        var val = Value.asFloat(stack.pop());
        stack.push(Value.toLong(OpcodeImpl.F32_TRUNC(val)));
    }
    ...
    private static void F64_STORE(MStack stack, Instance instance, Operands operands) {
        var value = Value.asDouble(stack.pop());
    ...
    private static void F32_STORE(MStack stack, Instance instance, Operands operands) {
        var value = Value.asFloat(stack.pop());
        var ptr = readMemPtr(stack, operands);
    ...
    private static void GLOBAL_SET(MStack stack, Instance instance, Operands operands) {
        ...
        var val = stack.pop();
        // TODO: unbox globals too!
        instance.writeGlobal(id, new Value(instance.global(id).getValue().type(), val));
    }
